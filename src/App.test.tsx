...
describe("App", () => {
  it("should render initial Todos", () => {
    ...
    expect(
      screen.getByText("Etwas sehr köstliches mit viel Käse und Bacon essen!")
    ).toBeDefined();
    expect(screen.getByTestId("check-Lecker essen")).not.toBeChecked();

    expect(screen.getByText("React lernen")).toBeDefined();
    expect(
      screen.getByText("Ganz viel wichtige React Grundlagen erlernen.")
    ).toBeDefined();
  ...
  it("should hide Todos with Show Todos button", () => {
    ...
    expect(
      screen.queryByText("Etwas sehr köstliches mit viel Käse und Bacon essen!")
    ).toBeNull();
    expect(screen.queryByText("check-Lecker essen")).toBeNull();

    expect(screen.queryByText("React lernen")).toBeNull();
    expect(
      screen.queryByText("Ganz viel wichtige React Grundlagen erlernen.")
    ).toBeNull();
  ...
  it("should filter todos and show only filtered todo", function () {
    render(<App />);
    
    const searchInput = screen.getByTestId("search-input");
    userEvent.type(searchInput, "React lernen");
    expect(screen.queryByText("React lernen")).toBeDefined();
  });
});
